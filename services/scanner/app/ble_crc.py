"""BLE Link Layer CRC-24, ported from the Ubertooth firmware's
`btle_crcgen_lut` (ubertooth/firmware/bluetooth_rxtx/bluetooth_le.c).

This exists because ubertooth-btle's host-side packet printer does *not*
verify CRC -- the "(valid)"/"(invalid)" tag on its "Advertising / AA ..."
lines only reflects whether the access-address bit pattern matched
(`access_address_ok` in libbtbb's bluetooth_le_packet.c), not whether the
payload's CRC-24 checks out. `-v1` (firmware CRC-verify mode) turned out to
have no effect on this capture path either -- confirmed empirically against
a real device (a WHOOP band whose advertised name should read
"WHOOP 5AG......." consistently; without this check, roughly a third of
captured packets came through with single/few-byte corrupted names or other
payload bytes, all of which fail this CRC check while every
correctly-spelled repeat passes it).

Advertising channel packets always use the fixed CRCInit 0x555555; the
generator here takes the bit-reversed form (0xAAAAAA) as its seed, matching
the firmware's own `crc_init_reversed` convention.
"""
...
ADV_CHANNEL_CRC_SEED_REVERSED = 0xAAAAAA


def btle_crc(data: bytes, seed: int = ADV_CHANNEL_CRC_SEED_REVERSED) -> int:
    state = seed & 0xFFFFFF
    for byte in data:
        key = byte ^ (state & 0xFF)
        state = (state >> 8) ^ _CRC_LUT[key]
    return state & 0xFFFFFF


def crc_ok(pdu: bytes, received_crc: bytes) -> bool:
    """pdu: PDU header + payload, as transmitted, excluding the 3-byte CRC
    trailer. received_crc: those 3 trailing bytes, in the order Ubertooth
    prints them (little-endian relative to the computed value)."""
    if len(received_crc) != 3:
        return False
    return btle_crc(pdu) == int.from_bytes(received_crc, "little")
